...
func (n *TokenRateNode) setMaxRate(maxRate uint64) {
	...
	refillPeriodNanos := minuteNanos / maxRate

	if refillPeriodNanos < minRefillRateNanos {
		refillPeriodNanos = minRefillRateNanos

		rawRate = float64(maxRate*minRefillRateNanos) / float64(minuteNanos)
	}

	adjust := offset(rawRate)

	n.maxRate = maxRate
	n.refillPeriodNanos = refillPeriodNanos * uint64(adjust)
